...
shared_ptr<RtcpReceivingSession> getRtcpReceivingSession(int id) {
	std::lock_guard lock(mutex);
	if (auto it = rtcpReceivingSessionMap.find(id); it != rtcpReceivingSessionMap.end()) {
		return it->second;
	} else {
...
void emblaceRtcpReceivingSession(shared_ptr<RtcpReceivingSession> ptr, int tr) {
	std::lock_guard lock(mutex);
...
int rtcChainRtcpReceivingSession(int tr) {
	return wrap([&] {
		...
		emblaceRtcpReceivingSession(session, tr);
		return RTC_ERR_SUCCESS;
	});
}
...
int rtcGetTrackSyncTimestamps(int id, uint64_t *rtpTimestamp, uint64_t *ntpTimestamp) {
	return wrap([&] {
		auto session = getRtcpReceivingSession(id);
